...
vector<vector<int>> RandomGraph(int VertexCount, int EdgeCount)
{
	...
	//Генерировать будем таким образом: в каждом узле номер соединяемого узла стоит на своём же месте - нужно будет для более быстройгенерации графа
		while (EdgeCount > 0)
		{
...
void PullTogetherVetices(vector<vector<int>>* Graph, vector<vector<int>>*
	EdgeWeights, int Vert1, int Vert2)
{
	...
	bool Check = 0;
	for (int j = 0; j < (*Graph)[(*Graph)[Vert1].size() - 1].size();
	j++)//т.е. мы проверяем последний добавленный элемент нужно ли в нем обновить вершины
	{
...
vector<int> AssistAlgShtorVagner(int VertexCount, int EdgeCount,
	vector<vector<int>> Graph, vector<vector<int>> EdgeWeights, int Missing)
{
	...
	for (int i = 0; i < Graph[Visited[0]].size(); i++)//добавим стартовые веса
		Weight[Graph[Visited[0]][i]] +=
		EdgeWeights[Visited[0]][Graph[Visited[0]][i]];
...
int AlgShtorVagner(vector<vector<int>> Graph, int VertexCount, int EdgeCount)
{
	...
	{
		Result = AssistAlgShtorVagner(VertexCount, EdgeCount,
			GraphCopy, EdgeWeights, i);

		PullTogetherVetices(&GraphCopy, &EdgeWeights, Result[1],
			Result[2]);
...
/*
void AssistCutVertexFinder(vector<vector<int>> Graph, int Start, vector<bool>*
Visited, vector<bool>* IsCutVertex, vector<int>* Discover, vector<int>* Low, int
Parent, int* Depth)
{
int Children = 0;
(*Visited)[Start] = 1;
...
{
Children++;
AssistCutVertexFinder(Graph, Graph[Start][i], Visited,
IsCutVertex, Discover, Low, Start, Depth);
...
(*Low)[Start] = min((*Low)[Start],
(*Discover)[Graph[Start][i]]);
...
for (int i = 0; i < Graph.size(); i++)
if (Visited[i] == 0)
AssistCutVertexFinder(Graph, i, &Visited, &IsCutVertex,
&Discover, &Low, Parent, &Depth);
...
void FindAllVertexCut(vector<vector<int>>* Graph, vector<int>* DelVer, int
	Max, int Start, int End)
{
	...
	for (int i = Start; i < End; i++)
	{
		...
		(*DelVer)[i] = Max - AlgShtorVagner(GraphCopy, GraphCopy.size(),
			EdgeCount);
		GraphCopy.clear();
	}
...
int main()
{
	...
	cout << "Граф задавать случайным образом(1), или с клавиатуры(0)?"
		<< endl;
	int check;
	cin >> check;
	if (check == 1) Graph = RandomGraph(VertexCount, EdgeCount);
	if (check == 0) Graph = CreatedGraph(VertexCount);
	cout << endl;
	PrintGraph(Graph);
	int Max;
	Max = AlgShtorVagner(Graph, VertexCount, EdgeCount);
	cout << endl << Max << " - максимальный разрез графа." << endl;
	if (Max == 1)
	{
	...
	thread T2(FindAllVertexCut, &Graph, &DelVer, Max, Graph.size() / 4,
		Graph.size() / 2);
	thread T3(FindAllVertexCut, &Graph, &DelVer, Max, Graph.size() / 2, (3 *
		Graph.size()) / 4);
	thread T4(FindAllVertexCut, &Graph, &DelVer, Max, (3 * Graph.size()) /
		4, Graph.size());
	T1.join();
	T4.join();
	T3.join();
	...
	thread T9(FindAllVertexCut, &Graph, &DelVer, Max, Graph.size() / 3, (2 *
		Graph.size()) / 3);
	thread T10(FindAllVertexCut, &Graph, &DelVer, Max, (2 * Graph.size()) /
		3, Graph.size());
	T8.join();
	T9.join();
	T10.join();
	End = chrono::steady_clock::now();
	Difference = chrono::duration_cast<std::chrono::milliseconds>(End -
		Begin);
	cout << "Время для трёх потоков: " << Difference.count() << " ms\n" <<
		endl;

	Begin = chrono::steady_clock::now();
	thread T6(FindAllVertexCut, &Graph, &DelVer, Max, 0, Graph.size() / 2);
	thread T7(FindAllVertexCut, &Graph, &DelVer, Max, Graph.size() / 2,
		Graph.size());
	T6.join();
	T7.join();
	End = chrono::steady_clock::now();
	Difference = chrono::duration_cast<std::chrono::milliseconds>(End -
		Begin);
	cout << "Время для двух потоков: " << Difference.count() << " ms\n" <<
		endl;
	...
	Difference = chrono::duration_cast<std::chrono::milliseconds>(End -
		Begin);
	cout << "Время для одного потока: " << Difference.count() << " ms\n" <<
		endl;
	...
	cout << endl;
	cout << "Искомые пары связностей(кол-во вершин,кол-во рёбер):"<<endl;
	...
	cout << endl;
	cout << "Таблиция для функции связности:" << endl;
	for (i = 0; i < ConnectivityPair.size(); i++)
	{
		for (j = 0; j < ConnectivityPair[i].size(); j++)
			cout << ConnectivityPair[i][j] << " ";
		cout << endl;
	}
